...
Асинхронное программирование - это классическая конструкция, позволяющая отделить выполнение единицы кода от основного приложения. Оно полезно, к примеру, 
при выполнении кода с высоким разбросом задержки, такие как операции ввода-вывода (I/O) или сетевые запросы.
Корутины представляют собой общий и эффективный способ поддержки асинхронного программирования. Они могут приостанавливать(suspend) и 
возобновлять(resume) выполнение кода, тем самым обеспечивая кооперативную(невытясняющую) многозадачность. Программирование с использованием сопрограмм похоже на стандартное многопоточное программирование, с важным отличием, что сопрограммы являются легковесными, а также переиспользуют нативные потоки. Это позволяет создавать сопрограмму
для небольших задач, объединять их с помощью структурированной многопоточности. 
Другим отличием от стандартных потоков является, также, то, что сопрограммы не требуют дополнительной синхронизации, так как явно передают исполнение, вместо этого полагаясь на отправление сообщений.
...
Таким образом, неэффективная реализация рандеву-каналов может привести к деградации производительности. В статье "Fast and Scalable Channels in Kotlin Coroutines"(Nikita Koval, Dan Alistarh, Roman Elizarov) предлагается эффективный алгоритм реализации рандеву-каналов с измерениями. 
...
Предлагается реализовать рандеву-канал на основе структуры данных, называемой бесконечным массивом. В нём мы будем хранить сообщения и сопрограммы, со счётчиками выполненных отправлений и получений. И отправление и получение в начале прибавляют свои счётчики, тем самым резервируя себе место в массиве. Каждая операция либо блокирует исполнение, либо проводит "рандеву" с запросом противоположного типа. Предполагается, что каждый элемент массива будет обработан ровно одной отправкой и ровно одним получением. 
```
...
< тут в скомпиленном латехе будет рисунок стейт машины из пепера >
- Рисунок 1: диаграмма конечного автомата, используемого операциями для синхронизации.
...
```
- Листинг 3: алгоритм для операции `receive`


# 4. Измерения

Данный алгоритм был реализован и интегрирован в стандартную библиотеку `Kotlin Coroutines`. Для сравнения производительности была использована стандартная реализация в библиотеке `Kotlin Coroutines`, а также алгоритмы Scherer et al.(TODO: поменять на русско-академический вариант), и Koval et al. 
...
Рисунок(2) показывает результаты измерения. Стоит заметить масштабируемость: в то время как пропускная способность остальных алгоритмов деградирует с увеличением количества потоков, предложенное решение продолжает масштабироваться.  
